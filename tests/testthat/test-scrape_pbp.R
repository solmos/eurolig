...
test_that("a wrong game_code throws an error", {
  expect_warning(scrape_pbp(1200, "E2018"),
                 "Unable to get data for games 1200")
})

test_that("a single game_code works", {
    expect_equal(length(scrape_pbp(172, "E2018")), 1)
})
...
test_that("only good requests are obtained", {
    ...
    expect_warning(scrape_pbp(game_codes, "E2018"),
                   "Unable to get data for games 1200")
    expect_equal(length(scrape_pbp(game_codes, "E2018")), 1)
})
